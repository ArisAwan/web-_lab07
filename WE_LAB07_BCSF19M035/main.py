...
app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+pymysql://root:ubai@localhost/lab#7'
# db = SQLAlchemy(app)
...
def contacts():
    if (request.method == 'POST'):
        ...
        if(title!="" or desc!=""):
            record = to_do_list(title=title, description=desc)
...
def output():
    ...
    try:
        table_content= db.session.query(to_do_list)
        lists=[]
        count=0
        ...
        print("Failed to display Record  from to_do_list : {}".format(error))
